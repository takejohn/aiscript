...
export const subEvaluator: Evaluator<Ast.Node & { type: 'sub' }> = {
	async evalAsync(context: AsyncEvaluationContext, node: Ast.Node & { type: 'sub' }, scope: Scope, callStack: readonly CallInfo[]): Promise<Value | Control> {
		return context._evalBinaryOperation('Core:sub', node.left, node.right, scope, callStack);
	},

	evalSync(context: SyncEvaluationContext, node: Ast.Node & { type: 'sub' }, scope: Scope, callStack: readonly CallInfo[]): Value | Control {
		return context._evalBinaryOperationSync('Core:sub', node.left, node.right, scope, callStack);
	},
};
