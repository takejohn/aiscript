...
export const andEvaluator: Evaluator<Ast.Node & { type: 'and' }> = {
	async evalAsync(context: AsyncEvaluationContext, node: Ast.Node & { type: 'and' }, scope: Scope, callStack: readonly CallInfo[]): Promise<Value | Control> {
	...
	evalSync(context: SyncEvaluationContext, node: Ast.Node & { type: 'and' }, scope: Scope, callStack: readonly CallInfo[]): Value | Control {
