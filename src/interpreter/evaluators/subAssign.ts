...
export const subAssignEvaluator: Evaluator<Ast.Node & { type: 'subAssign' }> = {
	async evalAsync(context: AsyncEvaluationContext, node: Ast.Node & { type: 'subAssign' }, scope: Scope, callStack: readonly CallInfo[]): Promise<Value | Control> {
	...
	evalSync(context: SyncEvaluationContext, node: Ast.Node & { type: 'subAssign' }, scope: Scope, callStack: readonly CallInfo[]): Value | Control {
