...
export const mulEvaluator: Evaluator<Ast.Node & { type: 'mul' }> = {
	async evalAsync(context: AsyncEvaluationContext, node: Ast.Node & { type: 'mul' }, scope: Scope, callStack: readonly CallInfo[]): Promise<Value | Control> {
		return context._evalBinaryOperation('Core:mul', node.left, node.right, scope, callStack);
	},

	evalSync(context: SyncEvaluationContext, node: Ast.Node & { type: 'mul' }, scope: Scope, callStack: readonly CallInfo[]): Value | Control {
		return context._evalBinaryOperationSync('Core:mul', node.left, node.right, scope, callStack);
	},
};
